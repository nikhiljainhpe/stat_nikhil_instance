...
<html>
  ...
  <body>
    ...
    <div id="content" class="report">
      ...
      <div id="interval‑settings" style="margin:1em 0;">
        <label for="interval-select"><strong>Poll every:</strong></label>
        ...
        <button id="interval-update">Update</button>
        <span id="interval-msg" style="margin-left:1em;color:green"></span>
      </div>

      <script type="text/javascript">
        jQuery(function($) {
          $('#interval-update').on('click', function(e) {
            e.preventDefault();
            var mins = parseInt($('#interval-select').val(), 10);
            $('#interval-msg').text('…saving…');
            $.ajax({
              url: 'http://localhost:5000/set-interval',
              method: 'POST',
              contentType: 'application/json',
              data: JSON.stringify({ interval: mins }),
            })
            .done(function(res) {
              $('#interval-msg').text(res.message);
            })
            .fail(function(xhr) {
              var err = (xhr.responseJSON && xhr.responseJSON.error)
                        ? xhr.responseJSON.error
                        : 'Unexpected error';
              $('#interval-msg').css('color','red').text('Error: ' + err);
            });
          });
        });
      </script>
