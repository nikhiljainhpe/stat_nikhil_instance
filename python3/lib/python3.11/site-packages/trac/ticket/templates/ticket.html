            ...
            <fieldset id="properties">
              ...
              <table class="trac-properties">
                ...
                <tr>
                  ...
                  <td class="fullrow" colspan="3">
  <!-- 1) keep the real summary field hidden so Trac still sees `field_summary` unchanged -->
                    <input
                      type="hidden"
                      id="field-summary"
                      name="field_summary"
                    ...
                    <!-- 2) flex‐wrapper to lay out dropdown + textbox on one line -->
                    <div style="display: flex; align-items: center; gap: 0.5em;">
                      <!-- dropdown first → appears on the left -->
                      <select id="field-summary-type">
                        <option value="">Select </option>
                        <option value="Replace">Replace</option>
                        <option value="Reseat">Reseat</option>
                        <option value="AC Power Rack">AC Power Rack</option>
                        <option value="Slot Swap">Slot Swap</option>
                        <option value="Other">Other</option>
                        <!-- …your other options… -->
                      </select>

                      <!-- textbox second → appears on the right -->
                      <input
                        type="text"
                        id="field-summary-text"
                        placeholder="Enter the rest of the summary"
                        style="flex: 1;" />
                    </div>

                    <!-- 3) your existing script stays the same -->
                    <script>
                      jQuery(function($) {
                        function syncSummary() {
                          var t   = $('#field-summary-type').val();
                          var txt = $('#field-summary-text').val();
                          $('#field-summary').val(t ? t + ': ' + txt : txt);
                        }
                        $('#field-summary-type, #field-summary-text')
                          .on('change keyup', syncSummary);

                        // on load, split current value back into the two fields…
                        var existing = $('#field-summary').val() || '';
                        if (existing.indexOf(': ') > -1) {
                          var parts = existing.split(': ');
                          $('#field-summary-type').val(parts.shift());
                          $('#field-summary-text').val(parts.join(': '));
                        } else {
                          $('#field-summary-text').val(existing);
                        }
                      });
                    </script>
                  </td>
        ...
        <div class="buttons">
          ...
          <!-- <input type="submit" name="Send to Slack" /> -->
          # if ticket.exists:
          <input type="button" value = "Add to Slack" class="trac-disable-on-submit trac-button" onclick="addToSlack()" />
          # endif
        </div>
      ...
      function addToSlack() {
        ...
        var summary = formData.get('field_summary') || '';
        var type = formData.get('field_type') || '';
        var priority = formData.get('field_priority') || '';
        var xname = formData.get('field_xname') || '';
        var name = formData.get('field_reporter') || '';
        
        var parts = [];

        if (xname) parts.push("Xname: " + xname);
        ...
        var text = parts.join(" | ");
        //console.log("this is test", newEls);
        

        fetch('http://localhost:5000/slack-proxy', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ text: text, name: name, ticket_url: ticket_url})
        })
        .then(response => {
          if (response.ok) {
            alert('Sent to Slack!');
