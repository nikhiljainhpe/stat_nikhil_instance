...
def fuzzy_find(value, options):
    ...
    matches = difflib.get_close_matches(value, list(options.keys()))

    if not matches:
        for l_opt, opt in list(options.items()):
