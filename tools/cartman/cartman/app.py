...
class CartmanApp(object):
    ...
    def get_property_options(self):
        ...
        options = {}
        for name, prop in list(self.get_properties().items()):
    ...
    def _format_headers(self, headers):
        ...
        """
        return "\n".join([": ".join(pair) for pair in list(headers.items())])
    ...
    def run_new(self, owner=None):
        ...
        body = em.get_payload()
        headers = OrderedDict(list(em.items()))
    ...
    def run_timeline(self, daysback=None):
        ...
        for item, description in text.extract_timeline_items(r.text):
            output.append("{}. {}".format(item, description))
